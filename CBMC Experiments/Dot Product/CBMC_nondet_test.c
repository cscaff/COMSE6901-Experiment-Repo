...
float dot_product_cuda(float* A, float* B, int n) {
    ...
    // We have to encode assumptions on our values so malloc does not freak out.
    ...
    float* d_partial = (float*)malloc(blocks * sizeof(float));
...
int main() {
    ...
    if (diff < 0.0f) diff = -diff;   // absolute value of difference
    assert(diff <= 1e-4f);
...
// How to run:
//
//   cbmc CBMC_nondet_test.c --unwind 5 --trace
//
// Expected outcome:
