...
// C++ Version
int matrix_mult_cpp(float** A, float** B, int n) {
...
int matrix_mult_cuda(float* h_A, float* h_B, int n) {
...
// "cbmc CBMC_undet_test.cpp --unwind 5 --trace"
...
int main() {
    ...
    for (int i = 0; i < N; ++i)
        for (int j = 0; j < N; ++j) {
            h_A[i*N + j] = nondet_float();
            h_B[i*N + j] = nondet_float();
        }

    // Copy matrix for C++ format:
    float** A = new float*[N];
    float** B = new float*[N];
    for (int i = 0; i < N; ++i) {
        A[i] = new float[N];
    ...
    // C++ Version
    float checksum_cpp = matrix_mult_cpp(A, B, N);

    // CUDA Version
    float checksum_cuda = matrix_mult_cuda(h_A, h_B, N);
